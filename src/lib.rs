...
pub fn solc_version() -> error::Result<String> {
    let output = Command::new("solc").args(&["--version"]).output().chain_err(|| "failed to run `solc --version`")?;
    let output_string = String::from_utf8(output.stdout).chain_err(|| "output from `solc --version` is not utf8")?;
    let version = output_string.lines().last().chain_err(|| "output from `solc --version` is empty")?.to_owned();
    Ok(version)
}
